import React from 'react';
import './footer.scss';

function Footer() {
    return (
        <footer id="ac-globalfooter">
            <div className="ac-gf-content">
                <nav
                    className="ac-gf-directory"
                    aria-label="Apple Directory"
                    role="navigation">
                    <div className="ac-gf-directory-column">
                        <div className="ac-gf-directory-column-section">
                            <label className="ac-gf-directory-column-section-label">
                                <h3 className="ac-gf-directory-column-section-title">
                                    Shop and Learn
                                </h3>
                            </label>
                            <ul className="ac-gf-directory-column-section-list">
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/us/shop/goto/pc">
                                        PC
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/console/">
                                        Cpnsole
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/mobile/">
                                        Mobile
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/tv-displays/">
                                        TV & Displays
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/audio/">
                                        Audio
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/community/">
                                        Community
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/support/">
                                        Support
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/store/">
                                        Store
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="ac-gf-directory-column">
                        <div className="ac-gf-directory-column-section">
                            <label className="ac-gf-directory-column-section-label">
                                <h3 className="ac-gf-directory-column-section-title">
                                    Explore
                                </h3>
                            </label>
                            <ul className="ac-gf-directory-column-section-list">
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/technology/">
                                        Technology
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/chrome-rgb/">
                                        Chroma RGB
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/concepts/">
                                        Concepts
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/esports/">
                                        Esports
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/apple-arcade/">
                                        Collabs
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="ac-gf-directory-column">
                        <div className="ac-gf-directory-column-section">
                            <label className="ac-gf-directory-column-section-label">
                                <h3 className="ac-gf-directory-column-section-title">
                                    Support
                                </h3>
                            </label>
                            <ul className="ac-gf-directory-column-section-list">
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="https://appleid.apple.com/us/">
                                        Get Help
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/us/shop/goto/registration-warranty">
                                        Registration & Warranty
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="home">
                                        MetaStore Support
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="MetaCare">
                                        MetaCare
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="metaid">
                                        Manage Meta ID
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="spvideos">
                                        Support Videos
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="accessibility-statement">
                                        Accessibility Statement
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="ac-gf-directory-column">
                        <div className="ac-gf-directory-column-section">
                            <label className="ac-gf-directory-column-section-label">
                                <h3 className="ac-gf-directory-column-section-title">
                                    Company
                                </h3>
                            </label>
                            <ul className="ac-gf-directory-column-section-list">
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/about-us/">
                                        About Us
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/careers/">
                                        Careers
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/press-room/">
                                        Press Room
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/newsroom/">
                                        Newsroom
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/us/shop/goto/order/list">
                                        Order Status
                                    </a>
                                </li>
                                <li className="ac-gf-directory-column-section-item">
                                    <a
                                        className="ac-gf-directory-column-section-link"
                                        href="/us/shop/goto/help">
                                        Shopping Help
                                    </a>
                                </li>
                            </ul>
                        </div>
                    </div>
                    <div className="ac-gf-directory-column-social-media">
                        <label>
                            <h2>Follow us on social media</h2>
                        </label>
                        <div className="ac-gf-directory-column-social-media-list">
                            <a href='instagram'>
                                <img
                                    src="/assets/images/social-media/instagram-social-icon.png"
                                    alt=""
                                />
                            </a>
                            <a href='facebook'>
                                <img
                                    src="/assets/images/social-media/facebook-social-icon.png"
                                    alt=""
                                />
                            </a>
                            <a href='twitter'>
                                <img
                                    src="/assets/images/social-media/twitter-social-icon.png"
                                    alt=""
                                />
                            </a>
                            <a href='youtube'>
                                <img
                                    src="/assets/images/social-media/youtube-social-icon.png"
                                    alt=""
                                />
                            </a>
                        </div>
                    </div>
                </nav>
                <div className='ac-gf-footer'>
                    <div className='ac-gf-footer-legal'>
                        <p className='ac-gf-footer-legal-copyright'>Copyright © 2022 WaltGear Inc. All rights reserved.</p>
                        <div className="ac-gf-footer-legal-links">
                            <a href="sitemap" className="ac-gf-footer-legal-link">Site Map</a>
                            <a href="legal-terms" className="ac-gf-footer-legal-link">Legal Terms</a>
                            <a href="privacy-policy" className="ac-gf-footer-legal-link">Privacy Policy</a>
                            <a href="cookies-policy" className="ac-gf-footer-legal-link">Cookies Policy</a>
                        </div>
                    </div>
                    <div className='ac-gf-footer-locale'>
                        <span className="ac-gf-footer-locale-country">United States</span>
                        <a href="locale-links" className='ac-gf-footer-locale-links'>Change Country</a>
                    </div>
                </div>
            </div>
        </footer>
    );
}

export default Footer;
